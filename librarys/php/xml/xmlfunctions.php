...
function buildXMLAttributes(&$values) {
    ...
    while (!empty($values)) {
        ...
        $attributes[$attName] = htmlspecialchars_decode($attValue, ENT_QUOTES);
...
class XMLNode {
    ...
    */
    function XMLNode($newName, $attributesList="", $strText="") {
        $this->name = $newName;
        ...
        if (is_array($arrTMP)) {
            ...
            $this->attributes = $arrTMP;
        }
        $arrTMP = false;  //Para liberar memoria

        if (strlen($strText)>0) {
        	$this->text = htmlspecialchars_decode($strText, ENT_QUOTES);
        	$strText = false; //Para liberar memoria
        }
    }
    ...
    function addChild($newName, $attributesList="", $strText="") {
        ...
        if (!is_array($this->childrenNames)) $this->childrenNames = array();
        if (!isset($this->childrenNames[$newName])) $this->childrenNames[$newName] = array();
    ...
    */
    function appendChild(&$child) {
        ...
        if (!is_array($this->childrenNames)) $this->childrenNames = array();
        if (!isset($this->childrenNames[$child->name])) $this->childrenNames[$child->name] = array();
    ...
    */
    function hasChildren() {
        if (is_array($this->children) && $this->children && count($this->children)>0) {
            return (true);
        }
        else {
            return (false);
    ...
    */
    ...
    function getAttribute($name) {
    	return (is_array($this->attributes) && isset($this->attributes[$name]))?$this->attributes[$name]:false;
    }
    ...
    function toString($boolAddNewLines = false, $boolShortSyntaxBaseNodes = true) {
	    ...
	    if (is_array($this->attributes)) {        
	        foreach ($this->attributes as $key => $value) {
	        	$strEscaped = safeprint($value, false);                
	            $s .= "{$strTMP}{$key}=\"{$strEscaped}\"";
                $strEscaped = false;//Liberar memoria
	        }
	    }
	    ...
	    elseif (strlen($this->text)>0) {
	    	$strEscaped = safeprint($this->text, false);
...
class XMLObject {
    ...
    */
    function XMLObject($strBuild, $boolWithAttributes = true, $strEncoding = "ISO-8859-1", $boolRemoveXMLHeader = false) {
...
*/
function safeprint($strString, $boolPrint = true)
{
    if ($boolPrint) {
