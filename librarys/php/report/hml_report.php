<?php
include_once "kernel/drivers/config_function.php";
define("REPORTER_INDEX", "report_ajax.php");
class hml_report {
    ...
    function hml_report($strQuery, $arrEncabezado = array(), $arrParametros = array(), $strModulo = false, $strVentana = false, $strReportKey = false, $boolDieEvents = true) {
        ...
        if (is_array($strQuery)) {
            ...
            $this->strQuery = $this->getQuery($strQuery);
    ...
    public function getQuery($strQuery){
        ...
        if($strEngine == "sqlsrv"){
            ...
            $strQuery = "SELECT * ";
            $strQuery .= "FROM (SELECT ROW_NUMBER() OVER(ORDER BY (select NULL as noorder)) AS RowNum, {$arrQuery[0]}
                                       FROM {$arrQuery[1]}
                                       ) as hml_report ";
            $strQuery .= "WHERE {$arrQuery[2]} ¿limit?";
            return $strQuery;
        }
    ...
    public function dibujarHML_RPT() {
        ...
        $strLink = REPORTER_INDEX;
        $strReturn .= <<<EOD
        <form id="form_rpt_hml-{$this->strReportKey}" name="form_rpt_hml-{$this->strReportKey}" action="{$strLink}?keyReport={$this->strReportKey}" method="post" target="_blank">
        ...
        <script type="text/javascript">
            ...
            function descargarDataS(page, boolPrimerScroll){
                ...
                if(!boolPararCarga){
                    ...
                    getData.strUrl = "{$strLink}?keyReport={$this->strReportKey}";
...
EOD;
    ...
    private function accionesHerramientas() {
        ...
        $strAddImg = "views/img/mas.png";
        $strRemoveImg = "views/img/menos.png";
        $strLink = REPORTER_INDEX;
        $strReturn = <<<EOD
...
EOD;
    ...
    private function stylePaginador() {
        ...
        $strReturn = "<link rel=\"stylesheet\" href=\"librarys/php/report/style_rpt.css\">";
